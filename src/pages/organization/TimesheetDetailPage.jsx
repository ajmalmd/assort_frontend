...
function getStatusBadge(status) {
  switch (status) {
    ...
    case "REJECTED":
      return "border border-zinc-300 bg-white text-muted-foreground";
      ...
      <div className="rounded-3xl border border-border bg-white p-6 sm:p-8">
        <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-2xl sm:text-3xl lg:text-4xl font-semibold tracking-tight text-foreground">
              {formattedDate}
            </h1>
      ...
      <div className="mt-12">
        ...
        <div className="space-y-10">
          {logs.map((log, index) => (
            ...
            >
              {/* TIME */}
              <div className="pt-1 text-sm text-foreground flex flex-col">
                <div>{log.start_time}</div>
                <div className="mt-1 text-muted-foreground">{log.end_time}</div>
              </div>
              ...
              <div className="relative hidden lg:flex justify-center">
                ...
                {index === logs.length - 1 && (
                  <div className="absolute bottom-0 h-10 w-4 bg-primary" />
                )}
              </div>

              {/* CONTENT */}
              <div className="rounded-3xl border border-border bg-white p-4 sm:p-6 transition hover:shadow-sm">
                <div className="flex flex-col lg:flex-row lg:items-stretch lg:justify-between gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <blockquote className="border-l border-zinc-300 pl-4 text-sm italic leading-relaxed text-muted-foreground">
                      “{log.remarks}”
                    </blockquote>
